...
uniform float height_scale : hint_range(0.00, 1.0, 0.01) = 0.5;
uniform float uv_scale : hint_range(1.0, 50.0, 1.0) = 20.0;
group_uniforms;
...
void fragment() {
	// uv is needed temporarily to prevent compilation error with injected snippets
	vec2 uv = UV;
	float fresnel = sqrt(1.0 - dot(NORMAL, VIEW));
	RIM = 0.1;
	ROUGHNESS = 0.01;
	vec3 screen_clr = texture(screen_tex, SCREEN_UV).rgb;
	vec3 final_clr = mix(screen_clr, color.rgb, 1.0 - dot(VIEW, NORMAL) * 2.0);
